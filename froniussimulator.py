...
modbus_port = 5222      #(Port über 5000 um sudo zu umgehen)
...
mqttc = mqtt.Client(client_id="FroniusSmartMeter",clean_session=False)
...
#    global rtime
    ...
    try:
        mp = float(message.payload.decode("utf-8"))
    except:
        return
    ...
    if message.topic == MQTT_TOPIC_CONSUMPTION:
        ...
        netzbezug = mp
        print(f"Netzbezug: {mp}")
    elif message.topic == MQTT_TOPIC_TOTAL_EXPORT:
        einspeisung = mp
        print(f"Einspeisung: {mp}")
...
def run_updating_server():
    ...
    print(f"### start server, listening on {modbus_port}")
    address = ("", modbus_port)
    StartTcpServer(
            context=a_context,  
            address=address,
